...
如果你是第一次搭这类服务，用一台腾讯云海外 CVM 做例子会更容易理解。这个例子同样适用于大多数海外 VPS。
...
- 如果你只是自己一台电脑用，优先选 `无域名 + SSH 隧道`
- 如果你想让多台自己的设备稳定访问，再选 `个人域名 + nginx`

腾讯云海外 CVM 的最小建议：

- 地域：香港、新加坡、东京、硅谷这类海外地域
- 系统：`Ubuntu 22.04 LTS`
- 安全组：
  - 只做 SSH 隧道时，放行 `22`
  - 做域名 + nginx 时，放行 `22`、`80`、`443`
